...
from model.AEQL.loss_va_a import MultimodalLoss as loss_f
...
parser.add_argument('--out_path', default='./checkpoints/a_train_AEQL/',
                    help='folder to output images and model checkpoints')
parser.add_argument('--load_path', default='./checkpoints/a_train_AEQLresnet_new.pth',
                    help="path to net (to continue training)")
parser.add_argument('--log_path', default='./logs/',
                    help='folder to output logs')
parser.add_argument('--experiment_name', default='a_train_AEQL_set1',
...
def train(model, trainloader, criterion, optimizer, epoch, logger):
    ...
    vision_specific_cls_out, audio_specific_cls_out, text_specific_cls_out, emotion_cls_output = model(f_inputs, e_inputs, v_inputs)
        ...
        print('[epoch:%d, iter:%d] Loss: %.03f | V_MAE: %.3f | V_MSE: %.3f | V_R2: %.3f | V_RMSE: %.3f '
              % (epoch, iter, sum_loss / (iter + 1), mae_k2, mse_k2, r2_k2, rmse_k2))
    logger.add_scalar("train_loss_v", sum_loss / count, global_step=epoch)

    
    logger.add_scalar("train_mae_a", sum_mae2 / count, global_step=epoch)
    logger.add_scalar("train_mse_a", sum_mse2 / count, global_step=epoch)
    logger.add_scalar("train_r2_a", sum_r22 / count, global_step=epoch)
    logger.add_scalar("train_rmse_a", sum_rmse2 / count, global_step=epoch)
...
def test(model, testloader, epoch, k_test, logger):
    ...
    vision_specific_cls_out, audio_specific_cls_out, text_specific_cls_out, emotion_cls_output = model(f_inputs, e_inputs, v_inputs)
        ...
        print('loss: %.3f, set%d, test_mae_v：%.3f, test_mse_v：%.3f, test_r2_v：%.3f, test_rmse_v：%.3f' 
              % (sum_loss / count, k_test, acc2, sum_mse2/count,sum_r22/count,sum_rmse2/count))
        print('set%d, min_v_mae: %.3f' % (k_test, max_acc2))
        ...
        logger.add_scalar("test_mae_a", sum_mae2 / count, global_step=epoch)
        logger.add_scalar("test_mse_a", sum_mse2 / count, global_step=epoch)
        logger.add_scalar("test_r2_a", sum_r22 / count, global_step=epoch)
        logger.add_scalar("test_rmse_a", sum_rmse2 / count, global_step=epoch)
